...
function priceFor(model) {
  ...
  for (const key in PRICES) if (model.startsWith(key)) return PRICES[key];
  return null;
}
...
class TokenAggregator {
  ...
  _ingestLine(line) {
    ...
    if (o.message.model) this.models.set(o.message.model, (this.models.get(o.message.model) || 0) + mTok);
  }
  ...
  snapshot() {
    ...
    const blank = () => ({ in: 0, out: 0, cw: 0, cr: 0, cost: 0 });
    ...
    for (const [k, b] of this.buckets) {
      ...
      if (k >= m30) { d30.in += b.in; d30.out += b.out; d30.cw += b.cw; d30.cr += b.cr; d30.cost += b.cost; }
